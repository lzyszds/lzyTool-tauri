...
export const hide = (id?: string) => {
  const _id = document.querySelector(id ?? 'body') as HTMLDivElement
  const loadingDiv = _id?.querySelector('#loadingDiv')
  if (loadingDiv) {
    //删除元素 loadingDiv
...
/* 
    id: 选择器
    text: loading文字
    weather: 是否有雨 (暂时懒得写请求了，以后再说)
  */
export const show = (id: string = 'body', text?: string, weather: 'clear' | 'rain' = 'clear') => {
  ...
  var _LoadHtml =
    `<div id="load-over"></div>
     <div id="load-layout">
        ${template[4]}
        <div class='loadtext'>${text == null ? text = "Loading..." : text = text}</div>
     </div>`
  ...
  //呈现loading效果
  div.innerHTML = _LoadHtml
  if (weather === 'clear') {
    const rain = div.querySelector('.rain') as HTMLDivElement
    rain.style.display = 'none'
  }

  let _id = document.querySelector(id) as HTMLBodyElement
  //如果当前所选元素大于等于页面可视高度，则loading界面设置全屏效果
  if (!_id) {
    let _id = document.querySelector(id) as HTMLBodyElement
    _id?.appendChild(div)
    let _over = document.querySelector('#load-over') as HTMLElement
    let _layout = document.querySelector('#load-layout') as HTMLElement
